        ...
        while (true)
        {
            ...
            if (!isPaused && !spawnCounter.IsFull())
            {
                ...
                if (spawnedItems.Count < maxItems)
                {
                    ...
                    if (randomInput != null)
                    {
                        GameObject item = Instantiate(randomInput.prefab, spawnPoint.position, Quaternion.identity);
    ...
    private IEnumerator MoveToEnd(GameObject item)
    {
        ...
        while (elapsedTime < travelTime)
        {
            if (!spawnCounter.IsFull() && !isPaused)
            {
                ...
                item.transform.position = Vector3.Lerp(startPosition, endPosition, elapsedTime / travelTime);
            }
            yield return null;
        }
